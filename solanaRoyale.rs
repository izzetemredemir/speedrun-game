...
pub fn create_token(
    ...
    let mut data = Token {
        ...
        owner_names: HashMap::new(), // Map owner address to their name
    };
...
pub fn transfer(
    ...
    let mut data = Token::deserialize(&token_account.data.borrow())?;

    // Check if the sender has sufficient balance
    let from_balance = data.balances.get_mut(from_account.key).ok_or(ProgramError::InvalidAccountData)?;
    if *from_balance < amount {
        return Err(ProgramError::Custom(1)); // InsufficientBalance error (custom error code)
    }

    // Perform the transfer
    let to_balance = data.balances.entry(to).or_insert(0);
    *from_balance -= amount;
    *to_balance += amount;
